...
import { Calendar, Clock, Plus, Users, X, Zap } from 'lucide-react';
import { useEffect, useState } from 'react';
import { appStore } from '@/lib/store';
import type { Class, Reservation, User } from '@/types';
...
export default function CalendarModule() {
  ...
  const getReservationsForDay = (day: Date) => {
    return reservations.filter(
      (r) => r.status === 'active' && isSameDay(new Date(r.classDate), day),
    );
  };
  ...
  const canReserve = (classData: Class, classDate: Date) => {
    ...
    // Check capacity
    const allReservations = appStore
      .getState()
      .users.flatMap((u) => u.reservations)
      .filter(
        (r) =>
          r.classId === classData.id &&
          r.status === 'active' &&
          isSameDay(new Date(r.classDate), classDate),
      );
  ...
  const handleReserve = (classData: Class, classDate: Date) => {
    ...
    const success = appStore.createReservation(
      currentUser.id,
      classData.id,
      classDate,
    );
    if (success) {
      ...
      alert(
        'No se pudo reservar la clase. Verifica que tengas membresía activa y que no hayas excedido los límites.',
      );
    }
  };
                ...
                {dayClasses.length === 0 ? (
                  ...
                  <div className="grid gap-3">
                    {dayClasses.map((classData) => {
                      ...
                      const userReservation = dayReservations.find(
                        (r) => r.classId === classData.id,
                      );
                      const isReserved = !!userReservation;

                      // Count total reservations for this class
                      const totalReservations = appStore
                        .getState()
                        .users.flatMap((u) => u.reservations)
                        .filter(
                          (r) =>
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <div className="flex items-center space-x-2">
                              <div
                                className={`px-2 py-1 rounded-full text-xs font-bold ${getIntensityColor(classData.intensity)} text-white`}
                              >
                          ...
                          <h4 className="text-gray-800 font-bold mb-1 text-lg">
                            {classData.name}
                          </h4>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            {isReserved ? (
                              <button
                                onClick={() =>
                                  handleCancelReservation(userReservation?.id)
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleReserve(classData, classDate)
                                }
