...
export default function CourseStartDateModal({
  ...
  const getInitialDate = () => {
    if (initialDate && /^\d{4}-\d{2}-\d{2}/.test(initialDate))
      return initialDate.split('T')[0].slice(0, 10);
    return getTodayStr();
  };
  ...
  const [minDate, setMinDate] = useState<string>(getTodayStr);
  ...
  useEffect(() => {
    ...
    const dateToUse =
      initialDate && /^\d{4}-\d{2}-\d{2}/.test(initialDate)
        ? initialDate.split('T')[0].slice(0, 10)
        : todayStr;
  ...
  const handleSubmit = async () => {
    ...
    if (!purchaseId && !orderId) {
      setError(
        'No se pudo identificar la compra. Recarga la página e intenta de nuevo.',
      );
      return;
    }
    ...
    try {
      ...
      // Actualizar la fecha de inicio vía API (usa sesión del servidor; más fiable que RLS en cliente)
      const res = await fetch('/api/student/purchase/start-date', {
        method: 'PATCH',
      ...
      if (!res.ok) {
        throw new Error(data.error || 'No se pudo guardar la fecha de inicio');
      }
      ...
      >
        ...
        <form onSubmit={onFormSubmit} noValidate>
              ...
              <div className="flex-1">
                ...
                <ul className="space-y-1 text-xs text-gray-700 dark:text-gray-300">
                  ...
                  <li className="flex items-start gap-2">
                    ...
                    <span>
                      <strong>Cada día se habilitará una nueva clase</strong>{' '}
                      para que puedas tomarla.
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <AlertCircle className="w-4 h-4 text-gray-500 dark:text-gray-400 mt-0.5 flex-shrink-0" />
          ...
          <button
            ...
            className="w-full py-3.5 px-6 rounded-xl border-2 border-gray-900 dark:border-white bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-semibold text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {isSubmitting ? (
              <>
          ...
          <p className="mt-3 text-[10px] text-center text-gray-500 dark:text-gray-400">
            Puedes cambiar la fecha de inicio antes de comenzar, pero una vez
            que empieces, no podrás modificarla.
          </p>
        </form>
      </div>
    </div>
