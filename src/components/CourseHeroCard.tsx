...
export default function CourseHeroCard({
  ...
  useEffect(() => {
    ...
    if (userProfile?.weight && userProfile?.target_weight && purchasedCourse) {
      ...
      if (totalToLose > 0) {
        ...
        const _totalLessons = purchasedCourse?.total_lessons || 1;
        const caloriesPerLesson =
          purchasedCourse?.estimated_calories_per_lesson || 100; // Valor por defecto
        const caloriesBurned = completedLessons * caloriesPerLesson;

        // Calcular progreso basado en calorías
        const calorieProgress = Math.min(
          100,
          (caloriesBurned / totalCaloriesNeeded) * 100,
        );
  ...
  }, [userProfile, purchasedCourse, nextLesson, streakDays]);
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
              ...
              <div className="space-y-3 mb-4">
                ...
                <div className="text-center text-xs text-gray-500 dark:text-gray-400">
                  {Math.min(100, Math.max(0, progressPercentage))}% hacia tu
                  meta
                </div>
                {purchasedCourse && (
                  <div className="text-center text-xs text-gray-500 dark:text-gray-400 mt-2">
                    {(purchasedCourse?.completed_lessons || 0) *
                      (purchasedCourse?.estimated_calories_per_lesson ||
                        100)}{' '}
                    cal quemadas
                  </div>
                )}
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {lessons?.slice(0, 4).map((lesson: any, _index: number) => {
                  ...
                  const isLocked =
                    lesson.lesson_order >
                    (purchasedCourse?.completed_lessons || 0) + 1;
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 mt-6 transition-all duration-500 ease-in-out animate-pulse">
            <div className="flex items-center space-x-2">
              ...
              <p className="text-sm text-gray-800 dark:text-gray-200 font-medium">
                ¡Excelente! Has comenzado tu transformación. Cada clase te
                acerca más a tu meta. ¡Sigue así!
              </p>
            </div>
          </div>
        )}
