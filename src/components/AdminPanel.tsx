...
import type { AdminStats, Class, ClassAttendance, User, Video } from '@/types';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-black/40 backdrop-blur-md rounded-2xl p-6 border border-white/20">
        ...
        <p className="text-white text-lg font-medium">
          Gestión completa de usuarios, clases, videos y estadísticas de
          RogerBox.
        </p>
      </div>
          ...
          <div>
            ...
            <div className="space-y-3 max-h-48 overflow-y-auto">
              {getActiveMemberships().map((user) => (
                <div
                  key={user.id}
                  className="bg-[#85ea10]/20 border border-[#85ea10] rounded-lg p-4"
                >
                  <div className="font-bold text-white text-lg">
                    {user.name}
                  </div>
                  <div className="text-sm text-[#85ea10] font-medium">
                    {user.weight}kg • Activa desde{' '}
                    {user.membership?.startDate.toLocaleDateString('es-CO')}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="space-y-3 max-h-48 overflow-y-auto">
              {getInactiveMemberships().map((user) => (
                <div
                  key={user.id}
                  className="bg-red-500/20 border border-red-400 rounded-lg p-4"
                >
                  <div className="font-bold text-white text-lg">
                    {user.name}
                  </div>
          ...
          {classAttendances.length === 0 ? (
            ...
            classAttendances.map((attendance) => (
              <div
                ...
                className="bg-black/60 rounded-lg p-6 border border-white/20"
              >
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="text-white font-medium text-lg">
                    Asistieron: {attendance.attendedCount} /{' '}
                    {attendance.totalCapacity}
                  </div>
                  <div className="text-white font-medium text-lg">
                    {format(attendance.classDate, "EEEE, d 'de' MMMM", {
                      locale: es,
                    })}
                  </div>
                </div>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {attendance.attendees.map((attendee, index) => (
                        <span
                          key={index}
                          className="bg-[#85ea10] text-black px-4 py-2 rounded-lg font-bold"
                        >
                          {attendee}
                        </span>
                      ))}
          ...
          {videos.map((video) => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[#85ea10] font-bold text-sm">
                  {video.purchaseCount} compras
                </span>
              </div>
