  ...
  try {
    ...
    const [
      ...
      supabaseAdmin
        .from('orders')
        .select(
          'id, customer_name, customer_email, amount, status, created_at, course:courses(title)',
        )
        .order('created_at', { ascending: false })
        .limit(5),
    ]);
    ...
    const approvedOrders = orders.filter(
      (o) => o.status === 'approved' || o.status === 'APPROVED',
    );
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      date.setDate(date.getDate() - i);
      const dateStr = date.toISOString().split('T')[0];
      const daySales = approvedOrders.filter((o) =>
    ...
    profiles.forEach((p) => {
      if (p.goals) {
        try {
          const goals =
            typeof p.goals === 'string' ? JSON.parse(p.goals) : p.goals;
    ...
    return NextResponse.json({
      kpis: {
        ...
        salesThisMonth: approvedOrders.filter(
