...
export async function GET(request: NextRequest) {
  try {
    ...
    const rows = logs ?? []
    const userIds = [...new Set(rows.map((l: { user_id: string | null }) => l.user_id).filter(Boolean))] as string[]
    ...
    if (userIds.length > 0) {
      const { data: profilesData } = await supabaseAdmin
        ...
        .in('id', userIds)
      if (profilesData) {
        userNamesMap = Object.fromEntries(profilesData.map((u: { id: string; name: string | null }) => [u.id, u.name ?? '']))
      }
    }
