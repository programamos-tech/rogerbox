...
async function updatePaymentsInvoiceNumbers() {
  try {
    ...
    console.log(
      `📝 Actualizando ${paymentsWithoutInvoice.length} pago(s) sin invoice_number...`,
    );
    ...
    for (const payment of paymentsWithoutInvoice) {
      ...
      if (updateError) {
        ...
        console.log(
          `✅ Pago ${payment.id} actualizado con invoice_number: ${invoiceNumber}`,
        );
      }
