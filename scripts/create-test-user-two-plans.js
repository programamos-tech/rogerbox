...
async function createTestUserWithTwoPlans() {
  try {
    ...
    if (plansError || !plans || plans.length < 2) {
      console.error(
        '❌ Error obteniendo planes o no hay suficientes planes activos',
      );
      console.log(
        '💡 Necesitas tener al menos 2 planes activos en la base de datos',
      );
      return;
    }

    console.log(
      `✅ Encontrados ${plans.length} planes:`,
      plans.map((p) => p.name),
    );
    ...
    console.log(
      `  Fin: ${plan1EndDate.toISOString().split('T')[0]} (vencido hace 7 días)`,
    );
    console.log(`Plan 2 (${plan2.name}):`);
    console.log(`  Inicio: ${plan2StartDate.toISOString().split('T')[0]}`);
    console.log(
      `  Fin: ${plan2EndDate.toISOString().split('T')[0]} (vence en 3 días)`,
    );
