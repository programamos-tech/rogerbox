...
async function main() {
  ...
  if (records.length === 0) {
    console.log(
      '❌ No se encontraron registros válidos. Revisa el formato del archivo.',
    );
    return;
  }
  ...
  for (const p of products) {
    ...
    console.log(
      `     Registros: ${p.count}, Precios: $${p.precios.join(', $')}`,
    );
  }
  ...
  // Estadísticas
  const clientsWithMultipleTx = clients.filter(
    (c) => c.transacciones.length > 1,
  );
  console.log(
    `   - Con múltiples transacciones: ${clientsWithMultipleTx.length}`,
  );
